  ...
  class Base
    ...
    class << self
      def init_single_models(*args)
        define_array_of_models(:array_single_models, args)
      end
      alias_method :init_models, :init_single_models

      def init_multiple_models(*args)
        define_array_of_models(:array_multiple_models, args)
      end
    ...
    def apply_parameters
      @array_single_models ||= array_single_models.reject do |model| 
        array_single_models_without_validates.include?(model) if self.respond_to?(:array_single_models_without_validates)
      end
      update_attributes
    ...
    def save
      if valid?
        models = Array.new(@array_single_models)
    ...
    def not_validate(*args)
      define_singleton_method(:array_single_models_without_validates) { args }
    end
    ...
    def attributes_is_present?(model)
      (get_names_form_attributes_of(model) & params.keys).present?
    end
    ...
    def all_attributes_is_empty?(model)
      is_empty = true
      array_symbols_of_attributes = (get_names_form_attributes_of(model) & params.keys).map { |attr| attr.to_sym }
    ...
    def save_last_model_if_not_associations(model)
      ...
      @array_single_models.each { |model2| association_trigger = true if get_association(model, model2) }
    ...
    def validation_models
      @array_single_models.each do |model|
